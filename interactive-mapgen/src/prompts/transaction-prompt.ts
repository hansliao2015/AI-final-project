...
2. **再繪製 mountain ，在其周圍補上 valley 作為過渡緩衝**
3. **再繪製 ocean ，在其周圍補上 water 作為過渡緩衝**
4. **最後才繪製 water、hill、lake 等**

### 操作說明：
- **在使用者描述的區域內，隨機生成對應的transaction，每個 transaction 代表一個地形點。**
- 你生成的點位，必須要有隨機性，不能看起來太過工整（太規律）。**禁止直線或均勻分布。**
- **你生成的點位，可以採用隨機的方向性，但須確保生成的地形大致的範圍符合使用者的描述。**
...
- **筆刷位置應非均勻排布、微偏移 ±60~100px，模擬自然重疊。**
...
- **外圍的邊界地形必須離邊界至少 300 像素，且要使用多個 transaction，並使用筆刷大小為xs或s**。
- **必須嚴格執行地形分層建構順序：**
...
- ocean可以使用中等的筆刷(m、s)，其他地形如mountain、valley、water等則使用較小的筆刷(s, m)。
